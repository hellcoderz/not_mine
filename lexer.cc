...
char* keywords[55] = {"","","","","","","","","",",","lambda","tau","->","@","and","function_form","fn","","in",".","let","where","aug","or","&","not","rl","gr","ge","ls","le","eq","ne","","","+","-","neg","*","/","**","","","","","gamma","<true>","<false>","<nil>","<dummy>","within","rec","=","<()>","<Y*>"};

struct lex_token { int token_type, token_len, token_loc;  char *token_name;  struct lex_token *next_node;};
struct exprAST { int token_type, node_value, token_loc;    struct exprAST *left, *right, *more;    char *node_name, *name_p;};
struct ExprAST_stack {struct exprAST *AST_node; struct ExprAST_stack *next_node;};

struct child_env {   struct env *num_now;  struct child_env *next_node;};
